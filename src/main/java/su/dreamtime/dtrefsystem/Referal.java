...
public class Referal {
    ...
    public static void use(final ProxiedPlayer sender, final String code) {
        Runnable runnable = () -> {
            ...
            if (ref == null) {
                sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "Упс.. Ваши данные не загружены! Перезайдите на сервер."));
                return;
            }
            if (ref.getReferrerId() != 0) {
                sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "Вы уже вводили код! Код можно ввести лишь один раз за всё время"));
                return;
            }
            ...
            try (ResultSet rs = DTRefSystem.getInstance().getDb().query(sql, sender.getName());
                ...
                if (!rsReferrer.next()) {
                    sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "Такого кода не существует!"));
                    return;
                    ...
                    if (ref == null) {
                        ...
                        sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "Вы не можете использовать свой же код!"));
                        return;
                    }
                    ...
                    if (ref.getPlaytimeSeconds() < needSeconds) {
                        sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "Перед тем, как вводить код приглашения, вы должны наиграть " + TimeUtils.getTimeString(needSeconds * 1000)));
                        sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "Осталось: " + TimeUtils.getTimeString((needSeconds - ref.getPlaytimeSeconds()) * 1000)));
                        return;
                    }
                    ...
                    sender.sendMessage(TextComponent.fromLegacyText(ChatColor.GREEN + "Отлично! Вы получили " + ChatColor.YELLOW + referalCoins + ChatColor.GREEN + " поинтов за приглашение!"));
                    ...
                    if (referrerPlayer != null) {
                        if (!DTRefSystem.getIgnoreList().contains(referrerPlayer.getName())) {
                            referrerPlayer.sendMessage(TextComponent.fromLegacyText(ChatColor.GREEN + "Отлично! Вы получили " + ChatColor.YELLOW + referrerCoins + ChatColor.GREEN + " поинтов за приглашение игрока " + ChatColor.YELLOW + sender.getName() + ChatColor.GREEN + "!"));
                        }
                    }
                    ...
                    sql = "UPDATE `dt_ref_referrers` SET `count_referals` = `count_referals` +1 WHERE `code` = ?";
                    DTRefSystem.getInstance().getDb().execute(sql, code);
